...
COMMENT ON SCHEMA fms IS '농장관리시스템(Farm Management System) DB';
...
COMMENT ON TABLE fms.chick_info IS '육계정보';
COMMENT ON COLUMN fms.chick_info.chick_no IS '육계번호';
COMMENT ON COLUMN fms.chick_info.breeds IS '품종';
COMMENT ON COLUMN fms.chick_info.gender IS '성별';
COMMENT ON COLUMN fms.chick_info.hatchday IS '부화일자';
COMMENT ON COLUMN fms.chick_info.egg_weight IS '종란무게';
COMMENT ON COLUMN fms.chick_info.vaccination1 IS '예방접종1';
COMMENT ON COLUMN fms.chick_info.vaccination2 IS '예방접종2';
COMMENT ON COLUMN fms.chick_info.farm IS '사육장';
